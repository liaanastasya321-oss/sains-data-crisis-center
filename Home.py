...
# --- FUNGSI AI PINTAR (AUTO-CHECK MODEL) ---
def panggil_ai_mesin(prompt_system, user_input):
    ...
    try:
        ...
        return f"🙏 AI sedang sibuk. (Error: {str(e)})"
    ...
    if not st.session_state['is_logged_in']:
        ...
        if len(raw_data) > 1:
            ...
            if 'i' in st.session_state:
                p_i = st.text_input("Perihal", value=st.session_state.p)
                t_i = st.text_input("Tujuan", value=st.session_state.t)
                i_i = st.text_area("Isi", value=st.session_state.i, height=300)
                if st.button("Cetak"):
                    st.download_button("Download", create_pdf("001/2026", "1", p_i, t_i, i_i), f"Surat_{data_t[1]}.pdf")
