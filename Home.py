...
def panggil_ai_aman(prompt_system, user_input):
    if "GEMINI_API_KEY" not in st.secrets:
        return "⚠️ API Key belum dipasang di Secrets."
    try:
        # Cek model yang tersedia di akun Lia agar tidak Error 404
        available_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        target_model = 'models/gemini-1.5-flash' if 'models/gemini-1.5-flash' in available_models else ('models/gemini-pro' if 'models/gemini-pro' in available_models else available_models[0])
...
def create_pdf(no_surat, lampiran, perihal, tujuan, isi_surat):
    ...
    pdf.set_font("Times", '', 10); pdf.set_x(0); pdf.cell(210, 5, "Sekretariat: Jl. Letkol Endro Suratmin, Sukarame, Bandar Lampung,", 0, 1, 'C')
            ...
            if sheet:
                ...
                if not hasil.empty:
                    for idx, row in hasil.iterrows():
                        ...
                        st.markdown(f'<div class="glass-card" style="border-left:5px solid {color}; text-align:left;"><h4>{row["Kategori"]}</h4><p>{row["Detail Keluhan"]}</p><div style="color:{color}; font-weight:bold;">{status}</div></div>', unsafe_allow_html=True)
...
# =========================================================
    ...
    if sheet:
        ...
        if not df.empty:
            ...
            st.plotly_chart(go.Figure(data=[go.Pie(labels=df['Kategori'].value_counts().index, values=df['Kategori'].value_counts().values, hole=.5)]), use_container_width=True)
...
# =========================================================
    ...
    if prompt := st.chat_input("Ketik pesan..."):
        ...
        with st.spinner("Berpikir..."):
            response = panggil_ai_aman("Kamu asisten virtual mahasiswa Sains Data UIN RIL. Jawab sopan.", prompt)
            ...
            if len(raw_data) > 1:
                ...
                if st.button("✨ 1. Buat Draft via AI"):
                    with st.spinner("AI Menulis..."):
                        raw_ai = panggil_ai_aman("Buat surat formal sekretaris himpunan. Format: PERIHAL|||TUJUAN|||ISI", f"Laporan: {data[5]}")
                        if "|||" in raw_ai:
                            st.session_state.p, st.session_state.t, st.session_state.i = raw_ai.split("|||")
                        else: st.session_state.i = raw_ai
                
                if 'i' in st.session_state:
                    p = st.text_input("Perihal", value=st.session_state.get('p',''))
                    t = st.text_input("Tujuan", value=st.session_state.get('t',''))
                    isi = st.text_area("Isi Surat", value=st.session_state.get('i',''), height=300)
                    if st.button("🖨️ 2. Cetak PDF"):
                        pdf_bytes = create_pdf("001/PIKM/2026", "1 Berkas", p, t, isi)
                        st.download_button("📥 Download PDF", pdf_bytes, f"Surat_{data[1]}.pdf", "application/pdf")
