...
/* CARDS & METRICS */
.glass-card { background: #ffffff; border-radius: 16px; padding: 20px; border: 1px solid #e2e8f0; text-align: center; height: 100%; transition: all 0.3s ease; }
...
/* BUTTONS */
div.stButton > button { background: linear-gradient(90deg, #2563eb, #1d4ed8); color: white; border: none; border-radius: 10px; font-weight: 700; width: 100%; padding: 10px; }
</style>
...
def panggil_ai(prompt_system, user_input):
    ...
    except: return "Gagal terhubung ke AI. Silakan cek koneksi atau API Key."

# --- PDF GENERATOR (STANDAR SURAT) ---
def create_pdf(no_surat, perihal, tujuan, isi_surat):
    ...
    pdf.set_font("Times", '', 10); pdf.set_x(0); pdf.cell(210, 5, "Sekretariat: Jl. Letkol Endro Suratmin, Sukarame, Bandar Lampung, Lampung 35131", 0, 1, 'C')
    ...
    pdf.set_x(120); now = datetime.datetime.now(); pdf.cell(0, 5, f"Bandar Lampung, {now.day} Februari {now.year}", 0, 1, 'C')
    pdf.set_x(120); pdf.cell(0, 5, "Hormat Kami,", 0, 1, 'C'); pdf.ln(20)
...
selected = option_menu(
    ...
    icons=["house", "exclamation-triangle", "search", "bar-chart", "robot", "lock"],
...
if selected == "Home":
    ...
    with st.form("form_lapor"):
        ...
        if submitted:
            if not keluhan: st.warning("Mohon isi keluhan Anda.")
            else:
                try:
    ...
    npm_input = st.text_input("Masukkan NPM untuk melacak:")
    if st.button("Cari"):
        data = sheet.get_all_records()
        res = [r for r in data if str(r['NPM']) == npm_input]
        if res:
            for r in res: st.info(f"Kategori: {r['Kategori']} | Status: {r['Status']}\n\nIsi: {r['Detail Keluhan']}")
    ...
    if sheet:
        ...
        if not data.empty:
            ...
            c2.markdown(f'<div class="glass-card"><div class="metric-value">{len(data[data["Status"]=="Pending"])}</div><div class="metric-label">Menunggu</div></div>', unsafe_allow_html=True)
            c3.markdown(f'<div class="glass-card"><div class="metric-value">{len(data[data["Status"]=="Selesai"])}</div><div class="metric-label">Selesai</div></div>', unsafe_allow_html=True)
            
            pie = go.Figure(data=[go.Pie(labels=data['Kategori'].value_counts().index, values=data['Kategori'].value_counts().values, hole=.3)])
    ...
    if prompt := st.chat_input("Tanya Sadas..."):
        ...
        ans = panggil_ai("Kamu asisten mahasiswa Sains Data. Jawab ramah.", prompt)
    ...
    if not st.session_state.auth:
        ...
        if len(raw) > 1:
            ...
            if 'i' in st.session_state:
                per = st.text_input("Perihal", value=st.session_state.p)
                tuj = st.text_input("Tujuan", value=st.session_state.t)
