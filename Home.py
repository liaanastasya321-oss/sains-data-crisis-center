...
# =========================================================
    ...
    if prompt := st.chat_input("Ketik pesanmu di sini..."):
        ...
        try:
            model_name = get_available_model(); model = genai.GenerativeModel(model_name)
            ...
            if "429" in str(e): response_text = "🙏 Kuota harian AI sedang penuh. Coba lagi nanti ya!"
            else: response_text = "🙏 Maaf, Sasda Bot sedang sinkronisasi model. Coba lagi sebentar."
        ...
        if sheet:
            try:
                ...
                if len(raw_data) > 1:
                    ...
                    st.write("---"); pilihan = [f"{i} | {r[1]} - {r[4]}" for i, r in enumerate(raw_data[1:], 2) if r[0].strip()]
                    if pilihan:
                        ...
                        with tab2:
                            ...
                            isi_lengkap = st.text_area("Isi Surat Lengkap", value=st.session_state.get('di', ''), height=300)
                            if st.button("🖨️ Cetak PDF Final"):
                                # Perbaikan di baris bawah ini: isi_lengkap
                                pdf_bytes = create_pdf(no_surat, "1 Berkas", per, tuj, isi_lengkap)
                                st.download_button("📥 Download", data=pdf_bytes, file_name=f"Surat_{data_t[1]}.pdf")
