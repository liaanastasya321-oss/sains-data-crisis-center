...
selected = option_menu(
    menu_title=None,
    options=["Home", "Lapor Masalah", "Cek Status", "Dashboard", "Sasda Bot", "Admin"], # TYPO FIXED: Sadas -> Sasda
        ...
        try:
            ...
            if len(data_info) > 0:
                for item in reversed(data_info):
                    ...
                    st.markdown(f"""
                    <div class="announce-card" style="border-left: 5px solid {border_color}; margin-bottom: 10px; padding: 10px; background: white; border-radius: 8px;">
            ...
            if submitted:
                if not keluhan: st.warning("Mohon isi deskripsi laporan.")
                    ...
                    with st.spinner("Mengirim..."):
                        ...
                        try:
                            if sheet is None: st.error("❌ Gagal Konek Database.")
                            else:
...
# =========================================================
# 7. HALAMAN: CEK STATUS & DASHBOARD (LOGIKA ASLI LIA)
# =========================================================
                    ...
                    if len(raw_data) > 1:
                        ...
                        if not hasil.empty:
                            for idx, row in hasil.iterrows():
                                ...
                                st.markdown(f"""<div class="glass-card" style="border-left:5px solid {color}; text-align:left; margin-bottom:10px;">
...
# =========================================================
    ...
    if prompt := st.chat_input("Ketik pesanmu di sini..."):
        ...
        if "GEMINI_API_KEY" in st.secrets:
            try:
                # FIXED: Panggil model dengan system_instruction yang benar
                ...
                # FIXED: Ambil history tanpa pesan terakhir user (karena nanti dikirim via send_message)
                history = []
                ...
                response = f"🙏 Maaf, server AI sedang sibuk. Error: {str(e)}"
        else: response = "⚠️ API Key Gemini belum dipasang."
...
# =========================================================
# 10. HALAMAN: ADMIN (FIXED AI DRAFT LOGIC)
# =========================================================
elif selected == "Admin":
                    ...
                    if pilihan_laporan:
                        ...
                        with tab_status:
                            ...
                            if st.button("💾 Simpan Status"):
                                ...
                                st.success(f"Status diubah!")
                                st.rerun()

                        with tab_surat:
                            st.write("#### 📝 Otomasi Draft Surat")
                            if st.button("✨ Hubungkan AI & Generate Draft"):
                                with st.spinner("AI merancang surat..."):
                                    # FIXED: Panggil AI dan simpan ke session state
                            ...
                            st.write("---")
                            # FIXED: Pakai 'key' agar input tidak ter-reset saat rerun
                            no_surat = st.text_input("Nomor Surat", value="001/PIKM-HMSD/II/2026", key="adm_no")
                            lampiran = st.text_input("Lampiran", value="1 Berkas", key="adm_lamp")
                            perihal_surat = st.text_input("Perihal", value=st.session_state.get('draft_perihal', ''), key="adm_peri")
                            tujuan_surat = st.text_input("Tujuan Surat", value=st.session_state.get('draft_tujuan', ''), key="adm_tuju")
                            isi_lengkap = st.text_area("Isi Surat", value=st.session_state.get('draft_isi', ''), height=300, key="adm_isi")
                            
                            if st.button("🖨️ Cetak PDF Final"):
                                pdf_bytes = create_pdf(no_surat, lampiran, perihal_surat, tujuan_surat, isi_lengkap)
                                st.download_button("📥 Download PDF", data=pdf_bytes, file_name=f"Surat_{data_terpilih[1]}.pdf", mime="application/pdf")
            except Exception as e: st.error(f"Error Database: {str(e)}")
