...
# --- KONFIGURASI AI GEMINI (VERSI STABIL: gemini-pro) ---
model = None
if "GEMINI_API_KEY" in st.secrets:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
    try:
        # Kita pakai 'gemini-pro' aja yang pasti ada
...
# =========================================================
# 9. HALAMAN: SADAS BOT (VERSI GEMINI PRO - ANTI ERROR) 🧠
# =========================================================
    ...
    if prompt := st.chat_input("Ketik pesanmu di sini..."):
        ...
        # --- LOGIKA AI GEMINI (PAKE GEMINI PRO) ---
        response = ""
        if model:
            try:
                ...
                response = f"⚠️ Maaf kak, ada masalah teknis: {str(e)}. (Tips: Pastikan requirements.txt sudah diupdate)"
        else:
