    ...
    if sheet:
        try:
            ...
            if len(raw_data) > 1:
                ...
                if not df.empty: st.dataframe(df[kolom_tampil], use_container_width=True, hide_index=True)
                else: st.info("Belum ada data.")
            else: st.info("⚠️ Data masih kosong.")
        except Exception as e: st.error(f"Error memuat dashboard: {str(e)}")
...
# =========================================================
    ...
    if prompt := st.chat_input("Ketik pesanmu di sini..."):
        ...
        if "GEMINI_API_KEY" in st.secrets:
            ...
            except Exception as e: response = "🙏 Maaf, server AI sedang sibuk."
                ...
                if len(raw_data) > 1:
                    ...
                    if pilihan_laporan:
                        ...
                        with tab_surat:
                            ...
                            tujuan_surat = st.text_input("Tujuan", value=st.session_state.get('draft_tujuan', ''))
                            isi_lengkap = st.text_area("Isi Surat", value=st.session_state.get('draft_isi', ''), height=300)
                            if st.button("🖨️ Cetak PDF"):
                                pdf_bytes = create_pdf(no_surat, lampiran, perihal_surat, tujuan_surat, isi_lengkap)
                                st.download_button(label="📥 Download PDF", data=pdf_bytes, file_name=f"Surat_{data_terpilih[1]}.pdf", mime="application/pdf")
                else: st.info("Belum ada data.")
